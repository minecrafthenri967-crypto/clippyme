...
def process_video_to_vertical(input_video, final_output_video, reframe_mode='auto',
    ...
    gaming_facecam_box: only used when reframe_mode == 'gaming'. ``None``
    (default) runs the usual _detect_gaming_facecam scan; a
    ``{"x", "y", "w", "h"}`` dict of 0..1 fractions (drawn by the user over
    their own screenshot of the stream — see the dashboard's facecam box
    picker) pins the facecam there directly via
    reframe_ops.resolve_facecam_box_from_fractions, skipping detection —
    ...
    if reframe_mode == 'disabled':
        ...
        if gaming_facecam_box:
            print("   🎮 Reframe mode: GAMING — split-screen, facecam pinned manually from a user-drawn box.")
        else:
    ...
    elif reframe_mode == 'gaming' and gaming_facecam_box:
        print("\n   🤖 Step 3: Using the manually-drawn facecam box — skipping detection.")
        facecam_box = resolve_facecam_box_from_fractions(
            gaming_facecam_box['x'], gaming_facecam_box['y'],
            gaming_facecam_box['w'], gaming_facecam_box['h'],
            ...
            while cap.isOpened() and not global_smooth:
                ...
                try:
                    if current_strategy == 'DISABLED':
                        ...
                        # (resolve_facecam_box_from_fractions, from a
                        # user-drawn box) — every frame reuses the same box,
