...
import { seedToggles, seedHookParams, seedSubtitleParams, seedLogoParams, seedBannerParams, seedPlayerImageParams, seedTeaserParams } from './seedClipParams.js';
...
export function clipStateToParams(state, preselections, clip) {
  return {
    ...
    playerImageParams: state?.playerImageParams || seedPlayerImageParams(preselections),
    teaserParams: state?.teaserParams || seedTeaserParams(preselections),
  };
}
...
export function buildClipParams(srcParams, targetClip, targetState) {
  ...
  return {
    ...
    playerImageParams: { ...(srcParams.playerImageParams || { position: 'center', size: 'M' }) },
    teaserParams: { ...(srcParams.teaserParams || {}) },
