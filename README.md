...
| `CLIPPYME_MAX_DOWNLOAD_HEIGHT` | `1080` | Download-quality CEILING in pixels of height; also settable in Settings → Downloads. `720`/`1080`/`1440`/`2160`, or `0` for uncapped ("true best" — can be 4K+, slower and much bigger). Only raises how much of a source's available resolution gets downloaded — it does nothing when the source itself tops out lower than the cap (common for many YouTube uploads and effectively all Twitch/Kick VODs). avc1 (H.264) is still preferred first for cheap CPU decode, but every rung of the format ladder now shares the same cap, so a video whose 1080p only exists as VP9/AV1 no longer silently falls back to a lower-resolution avc1 rendition. |
