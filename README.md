...
| `YTDLP_PROXY` | _(unset)_ | Upstream proxy for yt-dlp (SOCKS or HTTP(S) URL), e.g. an SSH `-D` tunnel back to a home connection or a residential proxy service. YouTube's "sign in to confirm you're not a bot" wall also weighs the request's own IP — datacenter/VPS ranges get flagged far more readily than the residential connection cookies were exported from, so this (not just re-uploading cookies) is what reduces how often the wall reappears on a server deployment. |
| `CLIPPYME_MAX_DOWNLOAD_HEIGHT` | `1080` | Download-quality CEILING in pixels of height; also settable in Settings → Downloads. `720`/`1080`/`1440`/`2160`, or `0` for uncapped ("true best" — can be 4K+, slower and much bigger). Only raises how much of a source's available resolution gets downloaded — it does nothing when the source itself tops out lower than the cap (common for many YouTube uploads and effectively all Twitch/Kick VODs). Codec preference (avc1/H.264, cheapest CPU decode) is a `format_sort` **tiebreak**, not a selector filter: YouTube serves avc1 only up to 1080p, so an avc1-filtered selector rung used to match a 1080p stream for every cap — making `1440`/`2160`/`0` fetch a byte-identical 1080p file. Resolution now wins, and h264 is still preferred where it can actually deliver that resolution. Above 1080p expect VP9/AV1, which cost more CPU to decode. |
| `CLIPPYME_MIN_OUTPUT_SHORT_EDGE` | `1080` | Render-canvas FLOOR in pixels (`reframe.py`'s `compute_output_dimensions`) — the counterpart to the ceiling above. The output canvas matches the source's native height 1:1 when that already clears this floor; below it, the canvas is instead upscaled (Lanczos) so the delivered clip's short edge never falls under a platform-safe minimum, regardless of how low-res the source is. `0` disables the floor (restores the old source-height-only sizing, which for a plain 1920x1080 landscape source produced only a 608x1080 9:16 crop — well under TikTok/Instagram's own recommended minimum, a plausible cause of a platform "low quality" flag). |
| `CLIPPYME_MAX_UPSCALE` | `2.0` | How far the reframe crop may be enlarged to fill the output canvas. Camera zoom tightens the crop, spending REAL source pixels; past this budget the difference is pure enlargement, which adds no detail. Caps the zoom accordingly (`reframe_ops.max_zoom_for_upscale`) — binding on a 1080p source, whose 9:16 crop is only ~607px wide, while a 4K source keeps the full 1.6x zoom range it can afford. `0` disables the budget (old unbounded behaviour). |
| `REFRAME_GAMING_FACECAM_FRACTION` | `0.45` | Share of the vertical canvas the facecam zone gets in `gaming` reframe mode; the centred gameplay crop fills the rest. It also sets that zone's aspect ratio, which `expand_box_to_aspect` reshapes the detected/manual facecam box to — so raising it keeps more of the source around the streamer rather than just stretching the same crop. The `seam` hook position tracks it, so hook and split never drift apart. Clamped to `0.15`–`0.75`. |
| `CLIPPYME_X264_CRF` | `18` | libx264 quality for the **delivered** clip (near-visually-lossless; lower = higher quality + bigger files, 0–51). |
| `CLIPPYME_X264_INTERMEDIATE_CRF` | `14` | libx264 quality for encodes whose output only feeds another encode — source slice, reframe master, and every compose layer pass. A delivered clip stacks roughly five generations and each one discards detail permanently, so the feeders run finer than the delivery CRF; clamped to never exceed `CLIPPYME_X264_CRF`. |
